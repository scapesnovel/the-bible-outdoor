...
2.5 **Verse-specific encouragement engine** — every Short/reflection is generated from the actual content of the picked verse: the engine detects whether it is a promise, command, or blessing, extracts its imagery (shepherd, light, refuge, wings, anchor…), and weaves a unique encouragement around it. CTA endings and long-form outros rotate too — no two videos ever say the same thing. An optional pre-generated LLM reflection bank (`data/reflections.json.gz`, built by `pipeline/gen_reflections.py`) upgrades this further; the composer is the always-works fallback so the channel never misses a day.
